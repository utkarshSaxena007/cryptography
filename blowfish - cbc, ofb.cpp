...
string f(string plaintext){
    ...
    for(int i=0; i<8; i+=2){
        int col = bin2int(hex2bin(plaintext.substr(i,2)));
        a[i/2] = S[i/2][col];
    }
...
void keyGenerate(string key){
    ...
    int j=0;
    for(int i=0; i<18; i++){
        P[i] = xor_(P[i], key.substr(j, 8));

        //cout<<"Subkey "<<i+1<<" : "<<P[i]<<endl;
        j = (j+8)%key.length();
    }
}

string round(int i, string s){

    string left = s.substr(0,8);
    string right = s.substr(8,8);

    left = xor_(left, P[i]);
    string fout = f(left);

    right = xor_(fout, right);

    //cout<<"round "<<(i+1)<<" : "<<right+left<<endl;;
...
string decrypt(string s){
    ...
    left = xor_(left, P[1]);
    right = xor_(right, P[0]);

    return right + left;
}

void cbc(string plaintext){

    cout<<"--CBC Mode--"<<endl;
    cout<<"Plaintext : "<<plaintext<<endl;
    cout<<"---Encrypting Blocks---"<<endl;

    vector<string> cipherblocks;
    ...
    for(int i=0; i<plaintext.length(); i+=16){
        ...
        block = xor_(block, cipherblocks.back());

        string cipher =  encrypt(block);
        cout<<cipher<<"   ";
        cipherblocks.push_back(cipher);
    }
    cout<<endl;

    cout<<"--Decrypting Blocks--"<<endl;

    for(int i=0; i<cipherblocks.size(); i++){

        string block = decrypt(cipherblocks[i]);

        if(i==0)
        block = xor_(block, iv);
        else
        block = xor_(block, cipherblocks[i-1]);
...
void ofb(string plaintext){
    ...
    int s = 8;
    string reg = iv;
    vector<string> cipblocks;

    for(int i=0; i<plaintext.length(); i+=s){

        string encreg = encrypt(reg);

        string block = xor_( plaintext.substr(i,s), encreg.substr(0,s));
        cout<<block<<"    ";
        cipblocks.push_back(block);
        reg = reg.substr(s, b-s) + encreg.substr(0,s);
    ...
    for(int i=0; i<cipblocks.size(); i++){

        string encreg = encrypt(reg);

        string text = xor_( cipblocks[i], encreg.substr(0,s));
        cout<<text<<"    ";
...
int main(){
    ...
    string plaintext;
    cout<<"Enter hexadecimal plaintext(length: multiple of 16)"<<endl;
    cin>>plaintext;

    string key;
    cout<<"Enter hexadecimal key(length: multiple of 8)"<<endl;
    cin>>key;
